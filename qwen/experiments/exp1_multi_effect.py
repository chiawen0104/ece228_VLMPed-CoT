...
def create_image_turn_last_n(folder_path, speed, pedestrain_dir,
                              ped_box_pixel, question, num_frames,
                              frame_step=2, max_images=16):
    """
    Step 1: 從原始 16 張以 frame_step=2 取樣 → 得到 8 幀（與原始 code 一致）
    Step 2: 從這 8 幀取最後 num_frames 幀送入模型

    num_frames=1 → [t8]
    num_frames=2 → [t7, t8]
    num_frames=4 → [t5, t6, t7, t8]
    num_frames=8 → [t1, t2, t3, t4, t5, t6, t7, t8]
    """
    ...
    # Step 1: frame_step=2 取樣，最多 max_images 張 → 8 幀
    ...
    # Step 2: 取最後 num_frames 幀
    selected_ped_files = sampled_ped_files[-num_frames:]
    selected_speed     = sampled_speed[-num_frames:]
    selected_ped_box   = sampled_ped_box[-num_frames:]
...
def predict_action_batch(test_image_folder, test_label_folder, test_speed_folder,
                         ...
                         question, num_frames, batch_size=4, max_test_samples=None,
                         speed_multiplier=1):
    true_labels = []
    predicted_labels = []
    ...
    print(f"\n{'='*60}")
    print(f"Experiment 1 — num_frames = {num_frames}")
    ...
    for folder_name in tqdm(test_folders):
        ...
        messages = [{"role": "user", "content": create_image_turn_last_n(
            image_path, speed, pedestrain_path, ped_box_pixel, question, num_frames
        )}]
...
if __name__ == "__main__":
    ...
    # ── Summary Table ──
    print("\n" + "="*70)
    print(f"{'Frames':>8} | {'Accuracy':>8} | {'Precision':>9} | "
          f"{'Recall':>6} | {'F1':>6} | {'AUC':>6}")
    print("-"*70)
    for r in all_results:
        print(f"{r['num_frames']:>8} | {r['accuracy']:>8.4f} | {r['precision']:>9.4f} | "
    ...
    output = {
        "experiment": "Experiment 1: Multi-Frame Effectiveness",
        "results": [
            {
    ...
    result_path = f"./experiments/results/exp1_{dataset}_wo-cot.json"
