...
def create_image_turn_ablation(folder_path, speed, pedestrain_dir,
                                ped_box_pixel, question, remove_idx,
                                frame_step=2, max_images=16):
    """
    Frame Ablation (Leave-One-Out)
    remove_idx: 要移除的幀的 index（0-based，對應 t1~t8）
    ...
    if remove_idx is not None:
        selected_ped_files = [f for i, f in enumerate(sampled_ped_files) if i != remove_idx]
        selected_speed     = [s for i, s in enumerate(sampled_speed)     if i != remove_idx]
        selected_ped_box   = [b for i, b in enumerate(sampled_ped_box)   if i != remove_idx]
    else:
...
def predict_action_batch(test_image_folder, test_label_folder, test_speed_folder,
                         ...
                         question, setting_name, remove_idx,
    ...
    print(f"\n{'='*60}")
    print(f"Experiment 2A — setting = {setting_name}")
    ...
    for folder_name in tqdm(test_folders):
        ...
        messages = [{"role": "user", "content": create_image_turn_ablation(
...
if __name__ == "__main__":
    ...
    # setting_name, remove_idx (0-based index 對應 t1~t8)
    settings = [
        ("full",      None),  # t1~t8
        ("remove_t1", 0),     # 移除 t1
        ("remove_t2", 1),     # 移除 t2
    ...
    all_results = []
    for setting_name, remove_idx in settings:
    ...
    print("\n" + "="*75)
    print(f"{'Setting':>12} | {'Accuracy':>8} | {'Precision':>9} | "
          f"{'Recall':>6} | {'F1':>6} | {'AUC':>6} | {'Drop':>6}")
    print("-"*75)
    for r in all_results:
        drop = full_acc - r["accuracy"]
        print(f"{r['setting']:>12} | {r['accuracy']:>8.4f} | {r['precision']:>9.4f} | "
    ...
    output = {
        "experiment": "Experiment 2A: Frame Ablation (Leave-One-Out)",
    ...
    result_path = f"./experiments/results/exp2a_{dataset}_wo-cot.json"
