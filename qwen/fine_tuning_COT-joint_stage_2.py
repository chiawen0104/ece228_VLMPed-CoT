...
class IntentDataset(Dataset):
    def __init__(self, image_dir, label_dir,speed_dir,pedestrain_dir,pedestrain_box, processor, tokenizer, question,
    ...
    def __getitem__(self, idx):
        ...
        # messages = [{"type": "image", "image": f"file://{img}","resized_height": 720, "resized_width": 1280}
        #             for img in [example["images"][0]]]

        # messages = [{"type": "image", "image": f"file://{img}","resized_height": 540, "resized_width": 960}
        #             for img in [example["images"][0]]]
        ...
        # ])
        messages.extend([
            {"type": "image", "image": f"file://{img}", "resized_height": 300, "resized_width": 150}
            for img in example["pedestrain"]
        ])

        messages.append({"type": "text", "text": full_prompt})  # ✅ 使用带速度的 prompt
...
peft_model = PeftModel.from_pretrained(model, model_id="./output/checkpoint-1500_stage1", is_trainable=True)
peft_model.enable_input_require_grads()  # 开启梯度检查点时，要执行该方法
...
class CustomStepSaveCallback(TrainerCallback):
    ...
    def on_step_end(self, args, state: TrainerState, control: TrainerControl, **kwargs):
        if state.global_step in self.save_steps and state.global_step not in self._saved_steps:
            save_path = os.path.join(self.output_dir, f"checkpoint-step-{state.global_step}_cot")
            kwargs["model"].save_pretrained(save_path)
            self._saved_steps.add(state.global_step)
            print(f">>> [Step {state.global_step}] Saved model to {save_path}")
