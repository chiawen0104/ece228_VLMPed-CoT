...
def predict_action_batch(test_image_folder, test_label_folder, test_speed_folder,
        ...
        if len(batch) == batch_size or folder_name == test_folders[-1]:
            ...
            for i, output in enumerate(outputs):
                ...
                if pred is None:
                    ...
                    continue
                print(f"[✅] {folder_names[i]} → 预测: {pred} | 真实: {gt_labels[i]} ")
    ...
    with open("../STF/evaluation_result_JAAD-beh.txt", "w") as f:
...
if __name__ == "__main__":
    # When using the JAAD dataset, remember to multiply the speed by 10 at line 137.
